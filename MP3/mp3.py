...
def houghTransform(img, theta_seg, rho_seg):
    ...
    theta_param = np.deg2rad(np.arange(0, 180, theta_seg))   # Ranges from 0 to 180 degrees in radian
    rho_param = np.arange(-tan_from_origin, tan_from_origin, rho_seg)    # Ranges from -max distance from origin to max from origin
...
def houghLine(origImage, accumulator, thetas, rhos):
    ...
    # Still under testing in my test.py  
    return result
...
if __name__ == '__main__':
    ...
    # lineImage = houghLine(origImage, accumulator, thetas, rhos)   # Working Process
